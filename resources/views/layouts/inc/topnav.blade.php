...
<header class="bg-white text-[#202224] py-4 px-10 flex justify-between items-center">
    <div class="text-[32px] font-medium flex flex-col sm:flex-row sm:items-center">
        ...
        <div class="mt-1 sm:mt-0 sm:ml-2">
            <span
                class="inline-block text-black text-xs rounded-full border-[#CDCFD2] bg-[#CDCFD2] px-3 py-1 font-light">
                {{ auth()->user()->getUserTypeTextAttribute() }}
            </span>
        </div>
    </div>
    ...
    <div x-data="{ open: false }" class="relative">
        <button @click="open = !open" class="flex items-center space-x-2 focus:outline-none">
            ...
            @else
                <svg class="w-8 h-8 rounded-full mr-5" viewBox="0 0 24 24" fill="none"
